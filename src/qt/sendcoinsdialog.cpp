...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients)
{
    ...
    if (model->privateKeysDisabled()) {
        questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(PACKAGE_NAME));
    } else {
