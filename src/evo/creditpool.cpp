...
static CreditPoolCb GetCbForBlock(const CBlockIndex* block_index, const Consensus::Params& consensusParams, size_t tailLength) {
    if (block_index == nullptr || tailLength == 0) return {0, 0};

    // recursively collect tail of blocks
    CreditPoolCb prev = GetCbForBlock(block_index->pprev, consensusParams, tailLength - 1);

    uint256 block_hash = block_index->GetBlockHash();

    constexpr size_t CreditPoolCacheSize = 1000;
    static unordered_lru_cache<uint256, CreditPoolCb, StaticSaltedHasher> creditPoolCache(CreditPoolCacheSize) GUARDED_BY(cs_cache);
...
{
    CreditPoolCb creditPoolCb = GetCbForBlock(pindexPrev, consensusParams, LimitBlocksToTrace);
